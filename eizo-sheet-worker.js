// 映像シート — 出馬表URLから各馬の過去レース映像リンク一覧を生成
// Cloudflare Workers 用 1ファイル構成 (UI + 取得API)
// デプロイ: Cloudflare ダッシュボード → Workers & Pages → Create Worker → コードをこれで全置換 → Deploy
...
export default {
  async fetch(req) {
    ...
    if (u.pathname === "/manifest.json") return manifest(u);
...
async function api(u) {
  try {
    ...
    if (!res.ok) return json({ error: `netkeiba応答 ${res.status}` }, 502);
    const html = await res.text();

    // レース見出し: <title>レース名 5走表示 | 2026年7月25日 札幌7R ...</title>
    let title = "";
    ...
    if (rows.length === 0) {
      // フォールバック: tr単位で分割して映像リンクを含む行だけ拾う
...
#status{margin:10px 14px;color:var(--sub);font-size:13px}
...
function history(){try{return JSON.parse(localStorage.getItem("es_hist")||"[]")}catch(e){return[]}}
function pushHistory(id,title){
...
async function load(q){
  ...
  try{
    ...
    pushHistory(d.raceId,(d.title||"").split("|")[1]?.trim()||d.title);
